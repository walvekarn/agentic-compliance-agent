...
from src.agentic_engine.testing.test_suite_engine import TestSuiteEngine
from src.agentic_engine.testing.test_scenario import TestScenario, ComplexityLevel
from src.agentic_engine.testing.failure_simulator import FailureSimulator
from src.agentic_engine.testing.failure_injection import FailureType
from src.agentic_engine.testing.benchmark_runner import BenchmarkRunner
...
async def get_agentic_engine_status():
    ...
    try:
        ...
        # STEP 4: Call OpenAI API with timeout using ReasoningEngine.llm
        try:
            status_analysis = await asyncio.wait_for(
                asyncio.to_thread(
                    reasoning_engine.llm.invoke,
                    status_prompt
                ),
                timeout=120.0  # 120 second timeout
            )
            
            status_summary = status_analysis.content.strip() if hasattr(status_analysis, 'content') else "System operational"
            print(f"[INFO] Status analysis completed at {timestamp}")
            
        except asyncio.TimeoutError:
            status_summary = "Status check timed out - system may be under load"
            print(f"[ERROR] Status endpoint timeout at {timestamp}")
        except Exception as e:
            ...
            try:
                ...
                try:
                    analysis = await asyncio.wait_for(
                        ...
                        timeout=30.0  # 30s timeout for AI analysis
                    )
                    ai_analysis = analysis.content.strip() if hasattr(analysis, 'content') else ""
                    results["summary"]["ai_analysis"] = ai_analysis
                    print(f"[INFO] OpenAI analysis completed for test suite at {timestamp}")
                except asyncio.TimeoutError:
                    print(f"[ERROR] Test suite AI analysis timeout at {timestamp}")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"[ERROR] Benchmark AI analysis timeout at {timestamp}")
                    results["summary"]["ai_analysis"] = None
                except Exception as e:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"[ERROR] Recovery AI analysis timeout at {timestamp}")
