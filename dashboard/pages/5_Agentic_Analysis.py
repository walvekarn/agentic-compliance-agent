...
if st.session_state.agentic_results:
        ...
        for i, step in enumerate(plan, 1):
            with st.container():
                ...
                <div class="step-card">
                    ...
                    <p><strong>Expected Tools:</strong> {', '.join(step.get('expected_tools', []))}</p>
                    <p><strong>Dependencies:</strong> {', '.join(step.get('dependencies', [])) if step.get('dependencies') else 'None'}</p>
                </div>
                """, unsafe_allow_html=True)
        
        if not plan:
            st.info("No plan generated yet. Run an analysis to see the execution plan.")
    ...
    with tab2:
        ...
        if not step_outputs:
            st.info("No step outputs available. Run an analysis to see execution results.")
    ...
    with tab3:
        ...
        if not reflections:
            st.info("No reflections available. Run an analysis to see quality evaluations.")
    
    # TAB 4: RECOMMENDATION
    with tab4:
        st.markdown("### Final Recommendation")
        
        final_recommendation = results.get("final_recommendation", "No recommendation available")
    ...
    with tab5:
        ...
        with col3:
            st.metric("Status", metrics.get("status", "unknown").upper())
        with col4:
        ...
        if all_tools_used:
            ...
            st.info("No tools were used in this analysis.")
        ...
        if reasoning_passes_total > 0:
            ...
            st.info("Single-pass reasoning was used for all steps.")
        
        st.markdown("---")
        st.markdown("### 📊 Detailed Metrics")
        
        # Success/Failure breakdown
        if metrics.get("successful_steps") is not None or metrics.get("failed_steps") is not None:
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Successful Steps", metrics.get("successful_steps", 0))
            with col2:
                st.metric("Failed Steps", metrics.get("failed_steps", 0))
        
        # Average step time
        if metrics.get("average_step_time"):
            st.metric("Average Step Time", f"{metrics.get('average_step_time', 0):.3f}s")
